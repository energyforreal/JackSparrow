...
_FUSION_DECISION_PATH = "mtf_fusion"
...
def _simple_atr(df: Any, period: int = 14) -> float:
    import pandas as pd
    ...
    atr = tr.rolling(int(period), min_periods=1).mean()
    if len(atr) < 2:
        return float(atr.iloc[-1] or 0.0)
    return float(atr.iloc[-2] or 0.0)


async def evaluate_fusion_prediction(
    *,
    symbol: str,
    ...
    """Single fused model → per-horizon gates → duration + ATR brackets."""
    import time
    ...
    from feature_store.transformer_btcusd.mtf_frames import build_10m_ohlcv_from_5m
    ...
    if df5.empty or len(df5) < 2:
        raise ValueError("Insufficient OHLCV data for fusion prediction")
    ...
    req_id = f"pred_fusion_{symbol}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    mctx: Dict[str, Any] = {
        **(context or {}),
        "v43_df5m": df5,
        "v43_df10m": df10,
    ...
    pred = model_response.predictions[0] if model_response.predictions else None
    pctx = pred.context if pred is not None and isinstance(pred.context, dict) else {}
    horizon_logits = dict(pctx.get("horizon_logits") or {})
    gates = dict(pctx.get("horizon_gates") or {})
    fusion_logits = list(pctx.get("tf_fusion_logits") or [])
    verdict = evaluate_horizon_forecast(
        horizon_logits,
        gates=gates,
        fusion_logits=fusion_logits,
        min_probability=float(getattr(settings, "transformer_min_confidence", 0.55) or 0.55),
        size_floor=float(getattr(settings, "transformer_size_floor", 0.35) or 0.35),
        max_position_size=float(getattr(settings, "max_position_size", 0.1) or 0.1),
    )
    ...
    execution_plan = build_fusion_execution_plan(verdict) if signal in _ENTRY_SIGNALS else {}
    atr = _simple_atr(df5)
    if execution_plan:
        execution_plan["atr_14"] = atr
        ...
        position_size = _default_position_size(
            confidence, float(execution_plan.get("size_fraction") or 0.0)
        )
    else:
        position_size = 0.0

    multi_tf_predictions = multi_tf_from_horizons(verdict)
    horizon_forecast = verdict.to_horizon_forecast()
    cross_tf_summary = {
        "decision_path": _FUSION_DECISION_PATH,
        "duration_key": verdict.duration_key,
        "duration_minutes": verdict.duration_minutes,
        "thesis": verdict.thesis,
        "tf_fusion_weights": verdict.fusion_weights,
        "dominant_tf": verdict.duration_key,
        "primary_execution_tf": verdict.duration_key,
    }
    ...
    market_context: Dict[str, Any] = {
        **mctx,
        "format": "jacksparrow_transformer_btcusd_mtf_fusion",
        "threshold": float(getattr(settings, "transformer_min_confidence", 0.55) or 0.55),
        "regime": verdict.thesis,
        ...
        "transformer_features": {"atr_14": atr},
        "decision_path": _FUSION_DECISION_PATH,
    ...
    policy_verdict = apply_portfolio_guard_to_verdict(
        policy_verdict, portfolio_guard, symbol=symbol
    )
    ...
    reasoning_chain["conclusion"] = (
        f"MTF fusion: {signal} duration={verdict.duration_key or 'none'} "
        f"thesis={verdict.thesis}"
    )
    reasoning_chain["steps"][0]["step_name"] = "mtf_fusion_forecast"
    ...
    feat_list = [
        MCPFeature(
            ...
            metadata={"transformer": True, "fusion": True},
...
async def evaluate_transformer_prediction(
    *,
    ...
    """Dispatch fused forecast (default) or emergency five-bundle synthesis."""
    path = str(
        getattr(settings, "transformer_decision_path", _FUSION_DECISION_PATH)
        or _FUSION_DECISION_PATH
    ).strip()
    if path != _DECISION_PATH:
        return await evaluate_fusion_prediction(
            symbol=symbol,
            context=context,
    ...
    return await _evaluate_synthesis_prediction(
        symbol=symbol,
        context=context,
        model_registry=model_registry,
        delta_client=delta_client,
...
async def _evaluate_synthesis_prediction(
    *,
    symbol: str,
    context: Dict[str, Any],
    model_registry: MCPModelRegistry,
