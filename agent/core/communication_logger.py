"""
Communication logger for tracking all agent inter-service communication.

Provides structured logging for WebSocket messages, command requests/responses,
and data flows between agent, backend, and frontend services.
"""
...
COMMUNICATION_LOG_FILE = LOGS_ROOT / "agent" / "communication.log"
...
def _get_logger() -> structlog.BoundLoggerBase:
    ...
    _logger = logging.getLogger("agent_communication")
    ...
    file_handler = RotatingFileHandler(
        ...
        maxBytes=5 * 1024 * 1024,  # 5MB (smaller for agent)
        backupCount=3,
        encoding="utf-8"
    )

    # structlog.wrap_logger with JSONRenderer produces full JSON lines; use simple formatter
    ...
    # Use wrap_logger to avoid overwriting the global structlog config (which would
    # break other modules using structlog.get_logger() when structlog calls the
    # factory with 0 args - causing "missing 1 required positional argument: 'name'").
...
def log_communication(
    ...
    target: Optional[str] = None,  # "frontend" | "backend" | "agent"
    ...
    **extra_fields
    ...
    Args:
        ...
        target: Target service ("frontend" | "backend" | "agent")
...
def log_backend_command(
    ...
    """Log backend command communication."""
    log_communication(
        direction=direction,
...
def generate_correlation_id(prefix: str = "agent") -> str:
