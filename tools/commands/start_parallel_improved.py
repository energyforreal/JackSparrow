#!/usr/bin/env python3
"""
Improved Parallel Process Manager for JackSparrow Trading Agent

Enhanced version of start_parallel.py with:
...
from tools.commands.utils import (
    Colors, get_safe_symbol, _flushed_print,
    load_root_env, get_python_executable, get_npm_executable,
    parse_database_url, parse_redis_url,
    check_port_accessible, check_postgres_running, check_redis_running
)
...
@asynccontextmanager
async def managed_services(project_root: Path, npm_cmd: str):
    """Context manager for managed services with proper cleanup."""
...
def setup_services(project_root: Path, npm_cmd: str) -> ParallelProcessManager:
    ...
    frontend_config = ServiceConfig(
        ...
        health_check_url=f"http://localhost:{frontend_port}"
    )
