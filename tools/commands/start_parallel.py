...
def load_root_env(project_root: Path):
    """Load env from root .env.example then .env (later overrides earlier).
    ...
    if not example_loaded and not env_loaded:
        print(f"{Colors.ERROR}No .env or .env.example at {project_root}.{Colors.RESET}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(f"  4. See docs/13-debugging.md for more help")
            sys.stdout.flush()
            sys.stderr.flush()
            sys.exit(1)
        ...
        try:
            ...
            if not manager.start_all():
                ...
                print(f"  5. See docs/13-debugging.md for more help")
                sys.stdout.flush()
                sys.stderr.flush()
                sys.exit(1)
        except Exception as e:
