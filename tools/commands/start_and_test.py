#!/usr/bin/env python3
"""
Orchestrated System Startup and Continuous Test Execution

Starts the JackSparrow trading agent system and runs comprehensive functionality tests
continuously while services are running. By default, runs tests every 5 minutes and
terminates immediately if any test failure is detected.

In continuous mode:
- Keeps all services (agent, backend, frontend) running indefinitely
- Runs tests every 5 minutes (configurable) in parallel with live services
- Logs all errors and issues in real-time
- Terminates immediately if any test failure (FAIL status) is detected
- Runs until manually stopped (Ctrl+C)

Usage:
    python tools/commands/start_and_test.py
    python tools/commands/start_and_test.py --no-continuous
    python tools/commands/start_and_test.py --no-startup --test-interval 60
    python tools/commands/start_and_test.py --groups infrastructure,core-services --verbose

Examples:
    # Run continuous testing (default - tests every 5 minutes)
    python tools/commands/start_and_test.py
    ...
    # Run specific test groups with custom interval (1 minute)
    python tools/commands/start_and_test.py --groups infrastructure,core-services --test-interval 60

    # Run tests in parallel mode with verbose output
    python tools/commands/start_and_test.py --test-mode parallel --verbose

    # Continue running even on failures (for debugging)
    python tools/commands/start_and_test.py --no-terminate-on-failure
"""
...
class OrchestratedTestRunner:
    ...
    def _signal_handler(self, signum: int, frame: Any) -> None:
        ...
        if not self.shutdown_event:
            ...
            # Schedule cleanup in event loop if running, otherwise rely on context manager
            try:
                loop = asyncio.get_running_loop()
                asyncio.create_task(self._async_cleanup())
            except RuntimeError:
                # No event loop running - cleanup will happen via context manager
                pass
