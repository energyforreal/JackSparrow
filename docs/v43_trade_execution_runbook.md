...
[`config.py`](../agent/core/config.py) and [`.env.example`](../.env.example) ship Tier 1–3 defaults tuned for **live profitability** (Jul 2026): `JACKSPARROW_V43_MIN_EDGE_COST_RATIO=0.75`, `V15_ADX_REGIME_FILTER_ENABLED=true` (ADX chop/trend filters on v43 entries), `TRADE_LIFECYCLE_ENABLED=true` with `TRADE_LIFECYCLE_LOG_ONLY=false`. Throughput-recovery relaxations still in place for policy: `AGENT_POLICY_MODE=ml_or_thesis`, lower trade-score floors (`AGENT_TRADE_SCORE_MIN`, `AGENT_TRADE_SCORE_MIN_GATED_ML_ADOPTION`; policy also uses `ENTRY_QUALITY_MIN_SCORE`), horizon fusion off, consensus/strategy agreement off, and `VOLATILITY_FILTER_ENABLED=false`. **Uncertainty and state-head policy gates run only when `JACKSPARROW_V43_STATE_HEADS_ENABLED=true`**; keep that `false` on NO-ML unless real state-head scores are supplied (orchestrator leaves `uncertainty_score` unset instead of defaulting to 0.05). Local overrides live in `.env` (Docker loads `.env.example` then `.env`).

**Quality-first layer** (see [Entry Quality and Lifecycle](entry-quality-and-lifecycle.md)):

- Gate 5 edge is **always computed** (`gate5_economic` on market context); permissive profiles may still set `final_long`/`final_short`, but the **economic dimension** and policy quality floor reject sub-cost edges.
- Flat hypothesis (`hypothesis_no_rule_fired`) blocks gated ML adoption in `ml_or_thesis`.
- Step 6 `adjudication_verdict` can force HOLD when policy would otherwise enter.
- Breakout extension vetoes use `AGENT_THESIS_BREAKOUT_BB_POS_MAX` / `AGENT_THESIS_BREAKOUT_BB_POS_SHORT_MIN` (no hard ADX ceiling).
