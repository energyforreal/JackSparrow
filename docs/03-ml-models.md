...
- **Promotion checklist (Pattern 3, multi-head)**: each `horizons.{key}.validation_metrics` uses `inference_path` = `"meta_calibrator"`; **per-horizon** `meta_auc` minimums: scalp_10m **0.54**, intraday_30m **0.56**, trend_1h **0.58**, swing_2h **0.60** (`validate_multihead_export_gates` with `V43_EXPORT_GATES_STRICT=true`); hard floor **0.52** always blocks. **Research-only**: `V43_EXPORT_GATES_STRICT=false` in the training notebook allows near-miss export with warnings — do not copy to `agent/model_storage/` until strict gates pass. Training labels use **cost-aware** suppression with horizon cost scale on 1h/2h (0.85× / 0.80× effective round-trip). Gross hit rates remain **warnings**, not hard blocks.  
...
|--------|--------------|----------|
| **v43 regression (Compose default today)** | `agent/model_storage/JackSparrow_v43_models_BTCUSD/` | Flat folder: **`metadata_v43.json`**, **`model_artifact_v43*.pkl`**; **`JackSparrowV43Node`**; **40** features; **`horizons`** scalp_10m (2), intraday_30m (6), trend_1h (12), swing_2h (24) on 5 m bars. |
