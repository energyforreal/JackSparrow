...
- **Pattern 3 meta-stacking (optional §4c in notebook)**: three-layer stack — (1) base **regressors** (LGBM + XGB + RF) produce stacked expected-return features; (2) **LGBMClassifier** `ensemble.meta` maps the stack to direction probability; (3) **Ridge** `ensemble.calibrator` maps probability back to **expected-return scale** so gate-5 (`agent/core/v43_signal_gates.py`) edge-vs-cost math is unchanged. **`EnsembleModel.predict()`** in [`agent/models/v43_pickle_shims.py`](../agent/models/v43_pickle_shims.py) always clips output to **[-0.10, 0.10]** as a safety backstop. Without `calibrator`, raw meta probabilities must not reach the gates (they would trivially pass gate-5).  
- **Promotion checklist (Pattern 3)**: `metadata_v43.json` → `model_architecture.meta_learner` = `"lgbm_classifier"` and `model_architecture.calibrator` = `"ridge"`; `validation_metrics.meta_auc` > **0.50**; regressor `validation_corr` > **0**; long/short gross hit rates > **50%** (export assertions in notebook §5).  
...
- **Checks**: `pytest tests/unit/test_jacksparrow_v43_contract.py tests/unit/test_jacksparrow_v43_mcp_row.py tests/unit/test_jacksparrow_v43_inference.py tests/unit/test_jack_sparrow_v43_node_ctx.py tests/unit/test_v43_pickle_shims.py`; `python scripts/smoke_test_v43.py`; manual [`jacksparrow_v43_smoke.md`](jacksparrow_v43_smoke.md).
...
1. Train/export **`metadata_v43.json`** and **`model_artifact_v43.pkl`** via `notebooks/jacksparrow_v43_delta_india_training.ipynb` (Colab zip ends up as a folder such as **`jacksparrow_v43_bundle`**). **Promote into the runtime bundle directory** (default in repo): **`agent/model_storage/JackSparrow_v43_models_BTCUSD/`** — copy at least those two files over the existing pair (PowerShell example: `Copy-Item -Path "$env:USERPROFILE\Downloads\jacksparrow_v43_bundle\*" -Destination ".\agent\model_storage\JackSparrow_v43_models_BTCUSD\" -Force`). Point **`MODEL_DIR`** / **`AGENT_MODEL_DIR`** at that same folder if you use a different path. Confirm the metadata includes the 120-bar embargo split, validation metrics, fee/slippage assumptions, data ranges, and provenance before promotion. Do **not** ship **`validation_metrics`: `null`** or placeholder **`split`** to production—those are structural placeholders only until you copy a full notebook export. Optional sidecars in that folder (**`feature_engineer.pkl`**, **`regime_models_v43.pkl`**) are ignored when the joblib artifact already embeds `feature_engineer` / regimes; remove or refresh them if you change training layout.
