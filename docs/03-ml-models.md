...
- **Promotion metadata**: include `split` (embargo **120 bars**), full `validation_metrics`, `data`, `provenance`, and — when §4c ran — `model_architecture` plus `inference_path: "meta_calibrator"`. Do not ship **`validation_metrics`: `null`** or placeholder **`split`** to production.  
...
- **Promotion checklist (Pattern 3)**: `model_architecture.meta_learner` = `"lgbm_classifier"` and `calibrator` = `"ridge"`; `validation_metrics.inference_path` = `"meta_calibrator"`; **meta-stack** `validation_corr` > **0**; `meta_auc` > **0.48** (notebook warns in **[0.48, 0.50)**); gross hit rates are **warnings** in §5 export, not hard blocks. Regressor-only stats live under `validation_metrics.regressor_baseline`.  
