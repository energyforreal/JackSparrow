...
#### Market signal generation triggers

Analysis is driven by **price movement** relative to a configurable threshold, not only on a fixed wall-clock schedule:

- **`PRICE_FLUCTUATION_THRESHOLD_PCT`**: When the absolute BTCUSD (or primary symbol) move from the last analysis anchor exceeds this percentage, the pipeline can trigger feature computation and model inference (default is environment-specific; check `agent/core/config.py` and `.env`).
- **Fast polling**: `MarketDataService` / Delta client use short poll intervals for spot checks; WebSocket streams may be used when enabled.
- **Multi-timeframe candles**: Configured `TIMEFRAMES` (commonly `3m,5m,15m`) supply bars for feature engineering; align exported model metadata with the timeframes you load.
...
- **Registry**: `feature_store/feature_registry.py` — `V15_FEATURES_5M`, `V15_FEATURES_15M`, `V15_FEATURES_BY_TF`. These lists must match the `features` field in each shipped `metadata_BTCUSD_*.json` (names and order) for the corresponding bundle; update the registry when you promote a new export whose selected feature set changed.
...
- **SL/TP at entry**: Default **`SL_TP_MODE=atr`**. The fused model puts duration-scaled ATR multipliers on `execution_plan` (`sl_atr_mult` / `tp_atr_mult`, `sl_tp_source=atr`). Shared math is [`agent/core/sl_tp.py`](../agent/core/sl_tp.py). Emergency synthesis can still use **`path_pred`** (15m MFE/MAE). Prices are rounded to the contract `tick_size` when known.
...
- Primary: duration-scaled ATR absolute prices on Delta mark-price brackets (`POST /v2/orders/bracket`)
- Shared math/tick rounding in `agent/core/sl_tp.py`; `sl_tp_source` tags positions (`atr` | `path_pred` | `fixed`)
...
- Persistent **paper trade audit** files: rotating `paper_trades.log` (`TRADE|`/`CLOSE|`) and append-only `signal_audit/live_audit.md`, with **IST-primary** timestamps and UTC companion fields — see [Logging – AI signal and action audit journal](12-logging.md#ai-signal-and-action-audit-journal) and [`reference/ai-signal-action-audit-log.md`](../reference/ai-signal-action-audit-log.md)
...
- Audit reports complement [Audit Report](15-audit-report.md); troubleshooting steps reference [Deployment](10-deployment.md#operations--maintenance-commands).
...
- Cash vs positions breakdown (available cash, margin used, open position count)
- Unrealized PnL
- Realized PnL
- **ROE %** on the PnL badge when margin is in use: unrealized PnL ÷ margin used (INR), not return on full notional; uses the same leverage assumptions as the backend portfolio service (see [Frontend](07-frontend.md#portfoliosummary-component))
- Performance metrics
