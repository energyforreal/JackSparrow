...
describe('ActivePositions table alignment', () => {
  const headers = ['Symbol', 'Side', 'Lev.', 'Lots', 'Entry', 'Mark', 'Liq.', 'PnL', 'Duration']

  it('renders nine column headers and nine cells per data row', () => {
    ...
    expect(headerRow!.querySelectorAll('th').length).toBe(9)

    const dataRow = table!.querySelector('tbody tr')
    expect(dataRow!.querySelectorAll('td').length).toBe(9)
    expect(screen.getByText('BTCUSD')).toBeInTheDocument()
    expect(screen.getByText('SHORT')).toBeInTheDocument()
  })

  it('loading skeleton uses nine columns', () => {
    const { container } = render(<ActivePositions isLoading />)
    const headerRow = container.querySelector('thead tr')
    expect(headerRow!.querySelectorAll('th').length).toBe(9)
    const skeletonRow = container.querySelector('tbody tr')
    expect(skeletonRow!.querySelectorAll('td').length).toBe(9)
  })
})
