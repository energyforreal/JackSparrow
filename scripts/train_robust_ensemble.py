...
try:
    from config.paths import get_project_root, XGBOOST_DIR
...
try:
    from feature_store.feature_pipeline import compute_features as _fs_compute_features
    from feature_store.feature_registry import get_feature_schema
...
# Resolution in minutes for returns_1h / returns_24h (from timeframe string)
def _resolution_minutes(timeframe: str) -> int:
    m = {"15m": 15, "30m": 30, "1h": 60, "2h": 120, "4h": 240}
    return m.get(timeframe, 15)


def compute_features(df: pd.DataFrame, resolution_minutes: int = 15) -> pd.DataFrame:
    if _USE_FEATURE_STORE:
        feat_df = _fs_compute_features(df, resolution_minutes=resolution_minutes, fill_invalid=True)
    else:
...
def _save_strict_artifacts(
    ...
    """Write strict filenames for agent: entry_meta.pkl, exit_model.pkl, etc."""
    save_art(entry_meta,   ens / "entry_meta.pkl")
    save_art(exit_meta,   ens / "exit_model.pkl")
    save_art(entry_base,  ens / "entry_base.pkl")
    save_art(exit_base,   ens / "exit_base.pkl")
    save_art(entry_scaler, ens / "entry_scaler.pkl")
    save_art(exit_scaler,  ens / "exit_scaler.pkl")
    log.info("  Wrote strict artifacts: entry_meta.pkl, exit_model.pkl, ...")


def _save_feature_schema(ens: Path, feature_list: List[str], training_stats: Dict[str, Dict]) -> None:
    """Write feature_schema.json with canonical list and training stats for drift."""
    if _USE_FEATURE_STORE:
        schema = get_feature_schema(include_stats=True, training_stats=training_stats)
    else:
        schema = {
            "version": "1.0",
    ...
    save_json(schema, ens / "feature_schema.json")


def _append_training_summary_row(
    ens: Path,
    tag: str,
    entry_met: Dict,
    exit_met: Dict,
    ...
    """Append one row to training_summary.csv (create if missing)."""
    row = {
        "tag": tag,
        "entry_accuracy": entry_met.get("accuracy", 0),
        "entry_auc_ovr": entry_met.get("auc_ovr", 0),
        "exit_auc": exit_met.get("auc", 0),
        "sharpe_proxy": round(sharpe, 4),
        "promoted": promoted,
    }
    path = ens / "training_summary.csv"
    df = pd.DataFrame([row])
    if path.exists():
...
def train_for_timeframe(
    ...
    res_min = _resolution_minutes(timeframe)
    feat_df = compute_features(df, resolution_minutes=res_min)
    ...
    # 12b. Feature drift statistics and training stats (for drift baseline)
    log.info("  Computing feature drift statistics …")
    drift_stats = compute_drift_stats(feat_df, FEATURE_LIST)
    training_stats = {col: {"mean": float(feat_df[col].mean()), "std": float(feat_df[col].std())} for col in FEATURE_LIST}
    ...
    # 13. Persist (artifacts to xgboost dir; strict names for agent compatibility)
    ext = _ARTIFACT_EXT
    out_xgb = output_dir if output_dir.name == "xgboost" else output_dir / "xgboost"
    out_xgb.mkdir(parents=True, exist_ok=True)
    ens = out_xgb  # write directly under xgboost (no robust_ensemble subdir for strict contract)
    ...
    # Strict artifact names for agent (primary timeframe only)
    if timeframe == cfg.timeframes[0]:
        _save_strict_artifacts(ens, entry_base, entry_meta, entry_scaler, exit_base, exit_meta, exit_scaler)
        _save_feature_schema(ens, FEATURE_LIST, training_stats)
    _append_training_summary_row(ens, tag, entry_met, exit_met, sharpe, promoted)
...
def main() -> None:
    ...
    out_xgb = PROJECT_ROOT / cfg.output_dir if str(cfg.output_dir).endswith("xgboost") else PROJECT_ROOT / cfg.output_dir / "xgboost"
    out_xgb.mkdir(parents=True, exist_ok=True)
    manifest = out_xgb / "MANIFEST.json"
    save_json(
        {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "git_commit": git_commit,
    ...
    training_metadata = {
        ...
        "artifact_dir": str(out_xgb),
        "strict_artifacts": ["entry_meta.pkl", "exit_model.pkl", "entry_base.pkl", "exit_base.pkl", "entry_scaler.pkl", "exit_scaler.pkl"],
        "results_summary": [{"tag": r["tag"], "entry_accuracy": r["entry_metrics"]["accuracy"], "exit_auc": r["exit_metrics"]["auc"]} for r in results],
    }
    save_json(training_metadata, out_xgb / "training_metadata.json")
    log.info(f"  training_metadata.json → {out_xgb / 'training_metadata.json'}")
    try:
        from ml_system import build_model_version_and_zip
        ver_doc = build_model_version_and_zip(model_dir=out_xgb, version=git_commit[:12] if git_commit else None)
        log.info(f"  model_version.json + models.zip → version={ver_doc.get('version')}")
    except Exception as e:
        log.warning(f"  Model registry/zip step skipped: {e}")
