...
def run(model_dir: str, symbol: str, timeframes: List[str]) -> bool:
    ens_path = Path(model_dir) / "robust_ensemble"
    all_pass = True
    nodes: dict = {}
    ...
    if not meta_paths:
        log.error("No metadata files found. Run train_robust_ensemble.py first.")
        return False

    # ─────────────────────────────────────────────────────────────────────────
    # Step 2 – Load nodes + warm-up
    # ─────────────────────────────────────────────────────────────────────────
    ...
    zero_pred = node.predict(zero_req)
