...
class RobustEnsembleNode(MCPModelNode):
    ...
    @classmethod
    def from_strict_artifacts(cls, base_dir: str | Path) -> "RobustEnsembleNode":
        """Load node from strict artifact names in base_dir (entry_meta.pkl, exit_model.pkl, etc.)."""
        base_dir = Path(base_dir)
        key = str(base_dir.resolve())
        if key in cls._cache:
            log.debug(f"RobustEnsembleNode: cache hit for strict [{key}]")
            return cls._cache[key]
        for name in ("entry_meta.pkl", "exit_model.pkl"):
            if not (base_dir / name).exists():
                raise FileNotFoundError(f"Strict artifact not found: {base_dir / name}")
        log.info(f"Loading RobustEnsembleNode from strict artifacts in {base_dir} …")
        entry_base   = _load(base_dir / "entry_base.pkl")
        entry_meta_m = _load(base_dir / "entry_meta.pkl")
        exit_base    = _load(base_dir / "exit_base.pkl")
        exit_meta_m  = _load(base_dir / "exit_model.pkl")
        try:
            entry_scaler = _load(base_dir / "entry_scaler.pkl")
        except (FileNotFoundError, OSError):
            entry_scaler = None
        try:
            exit_scaler = _load(base_dir / "exit_scaler.pkl")
        except (FileNotFoundError, OSError):
            exit_scaler = None
        drift_stats = {}
        for p in (base_dir / "feature_drift_BTCUSD_1h.json", base_dir / "feature_drift_BTCUSD_15m.json"):
            if p.exists():
                try:
                    with open(p) as f:
                    ...
                    pass
        regime_clf = None
        for meta_path in base_dir.glob("regime_metadata_*.json"):
            try:
                from agent.models.regime_classifier import RegimeClassifier
                regime_clf = RegimeClassifier.from_metadata(meta_path)
                break
            except Exception:
                continue
        meta = {
            "model_name": "robust_ensemble",
            "version": "3.0.0",
    ...
    async def initialize(self) -> None:
        """No-op; model is loaded in from_strict_artifacts/from_metadata."""
        pass

    async def get_health_status(self) -> Dict[str, Any]:
        """Return health status for registry."""
        return {
            "status": self._health_status,
            "model_name": self.model_name,
            "call_count": self._call_count,
            "avg_ms": self._total_ms / max(1, self._call_count),
        }

    def _predict_sync(self, request: MCPModelRequest) -> MCPModelPrediction:
        """Sync prediction (used by warm_up and by async predict)."""
    ...
    async def predict(self, request: MCPModelRequest) -> MCPModelPrediction:
        """Async wrapper for MCP protocol."""
        return self._predict_sync(request)
