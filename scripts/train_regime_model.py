...
def train_regime_for_timeframe(
    ...
    # 6. Train regime models (output to xgboost dir for strict contract)
    ...
    meta    = trainer.train(feat_df, close, entry_lbl, tag, out_xgb)
    ...
    try:
        ...
        classifier  = RegimeClassifier.from_metadata(
            out_xgb / f"regime_metadata_{tag}.json"
        )
    ...
    baseline_path = out_xgb / f"regime_metadata_{tag}.json"
    ...
    meta_path = out_xgb / f"regime_metadata_{tag}.json"
    ...
    # Strict artifact: regime_model.pkl for primary timeframe
    if timeframe == cfg.timeframes[0]:
        try:
            import joblib
            regime_path = out_xgb / f"regime_model_{tag}.joblib"
            if regime_path.exists():
                model = joblib.load(regime_path)
                joblib.dump(model, out_xgb / "regime_model.pkl")
                log.info("  Wrote strict artifact: regime_model.pkl")
        except Exception as e:
            log.warning(f"  Could not write regime_model.pkl: {e}")
