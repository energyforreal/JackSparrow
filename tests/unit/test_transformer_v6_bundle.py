...
def _write_bundle(tmp_path: Path, contract: str) -> Path:
    meta = {
        "model_name": "jacksparrow_transformer_BTCUSD_15m",
        "version": "transformer_per_tf_v1",
        "model_family": model_family_for_resolution("15m"),
    ...
    (tmp_path / onnx_filename_for_resolution("15m")).write_bytes(b"not-onnx")
    return tmp_path / TRANSFORMER_METADATA_FILENAME


def test_from_metadata_path_accepts_v6_contract(tmp_path: Path) -> None:
    node = TransformerModelNode.from_metadata_path(
        _write_bundle(tmp_path, FEATURE_CONTRACT_VERSION_V6)
    )
    assert node.resolution == "15m"


def test_from_metadata_path_accepts_v7_contract(tmp_path: Path) -> None:
    node = TransformerModelNode.from_metadata_path(
        _write_bundle(tmp_path, FEATURE_CONTRACT_VERSION_V7)
    )
    assert node.resolution == "15m"
