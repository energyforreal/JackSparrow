"""Causal 15m feature engineering for the BTCUSD transformer."""
...
from feature_store.transformer_btcusd_15m.contract import FEATURE_COLS
from feature_store.transformer_btcusd_15m.htf_features import (
...
def prepare_raw_frame(
    ...
    *,
    ...
    """Normalize agent/OHLCV frames to the notebook's ``raw_df`` schema."""
    out = df.copy()
...
def add_features(
    ...
    *,
    ...
    """Compute causal features (identical to the Colab ``add_features`` cell)."""
    out = df.copy()
    ...
    out["rv_16"] = out["ret_1"].rolling(16).std()
    out["rv_96"] = out["ret_1"].rolling(96).std()
    ...
    out["obv_z"] = (obv_raw - obv_raw.rolling(96).mean()) / (
        obv_raw.rolling(96).std() + 1e-9
    )

    out["vol_z"] = (out["volume"] - out["volume"].rolling(96).mean()) / (
        out["volume"].rolling(96).std() + 1e-9
    )
    ...
    if "open_interest" in out.columns:
        out["oi_z"] = (out["open_interest"] - out["open_interest"].rolling(96).mean()) / (
            out["open_interest"].rolling(96).std() + 1e-9
        )
    else:
        out["oi_z"] = np.nan

    ret_2 = out["close"].pct_change(2)
    fund_rate = out["funding_rate"].fillna(0.0)
    funding_deriv = compute_funding_derivatives(fund_rate, ret_2)
    for col in funding_deriv.columns:
        out[col] = funding_deriv[col].values

    h1_feats = compute_h1_features(out)
    for col in h1_feats.columns:
        out[col] = h1_feats[col].values

    oi_deriv = compute_oi_derivatives(out)
...
def build_feature_matrix(
    ...
    *,
    ...
    feat = add_features(df, atr_period=atr_period)
    if dropna:
        feat = feat.dropna().reset_index(drop=True)
    return feat


def latest_closed_feature_row(
